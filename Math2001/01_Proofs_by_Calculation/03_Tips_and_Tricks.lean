...
example {z : ℝ} (h1 : z ^ 2 - 2 = 0) : z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1 = 3 :=
  calc
    z ^ 4 - z ^ 3 - z ^ 2 + 2 * z + 1
    _ = ((z^2-2)+2)^2 - (((z^2-2)+2) * z) - ((z^2-2)+2) + 2 * z +1 := by ring
...
example {a b c : ℝ} (h1 : a + 2 * b + 3 * c = 7) (h2 : b + 2 * c = 3)
  ...
  calc
    a
    _ = (a+2*b+3*c) - 2*((b+2*c)-2*c)-3*c := by ring
...
example {t : ℚ} (ht : t ^ 2 - 4 = 0) :
  ...
  calc
    ...
    _ = ((t^2-4)+4)^2 + 3 * ((t^2-4)+4) * t - 3 * ((t^2-4)+4) - 2*t -2 := by ring
